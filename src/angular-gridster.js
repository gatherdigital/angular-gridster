/*
 * version 0.13.5
 * modified last on 04-18-17 by Matt Kennedy
 * allow individual panels to be resizable or draggable
 */
...
(function(root, factory) {
	...
	return angular.module('gridster', [])
		...
		.constant('gridsterConfig', {
			...
			floatingUp: true, // whether to automatically float items up the stack
			floatingDown: false, // whether to automatically float items down the stack
		...
		.controller('GridsterCtrl', ['gridsterConfig', '$timeout',
			function(gridsterConfig, $timeout) {
				...
				this.floatItemsUp = function() {
					if (this.floatingUp === false || this.floatingDown === true) {
						return;
					}
					for (var rowIndex = 0, l = this.grid.length; rowIndex < l; ++rowIndex) {
						var columns = this.grid[rowIndex];
				...
				/**
				 * Moves all items down as much as possible
				 */
				this.floatItemsDown = function() {
					if (this.floatingDown === false || this.floatingUp === true) {
						return;
					}
				...
				/**
				 * Float an item down to the most suitable row
				 *
				 * @param {Object} item The item to move
				 */
				this.floatItemDown = function(item) {
					if (this.floatingDown === false || this.floatingUp === true) {
						return;
					}
					var colIndex = item.col,
					...
					while (rowIndex > -1) {
						...
						var items = this.getItems(rowIndex, colIndex, sizeX, sizeY, item);
						if (items.length === 0) {
							// if no items found at this index, then break
							break;
						}
					}
					if (bestRow !== null) {
				...
				return {
					...
					compile: function($tplElem) {
						...
						return function(scope, $elem, attrs, gridster) {
							...
							$timeout(function() {
								scope.$watch('gridster.floatingUp', function() {
									gridster.floatItemsUp();
								});
								gridster.loaded = true;
							}, 100);

							// allow a little time to place items before floating down
							$timeout(function() {
								scope.$watch('gridster.floatingDown', function() {
									gridster.floatItemsDown();
								});
								gridster.loaded = true;
							}, 100);
			...
			function($document, $window, GridsterTouch) {
				function GridsterDraggable($el, scope, gridster, item, itemOptions) {
					...
					function mouseDown(e) {
						...
						if (!gridster.draggable || !item.draggable) {
							return;
						}
		...
		.factory('GridsterResizable', ['GridsterTouch', function(GridsterTouch) {
			function GridsterResizable($el, scope, gridster, item, itemOptions) {
				...
				function ResizeHandle(handleClass) {
					...
					function mouseDown(e) {
						if (!gridster.resizable || !item.resizable) {
							return;
						}
				...
				// add resize handles if item is resizable
				if (gridster.resizable && item.resizable) {
					...
					link: function(scope, $el, attrs, controllers) {
						...
						var aspectFn = function(aspect) {
							...
							if (typeof val === 'number') {
								item[aspect] = val;
							} else if (aspect === 'resizable' || aspect === 'draggable') {
								item[aspect] = val;
							}
						};
